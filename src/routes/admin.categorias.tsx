...
function AdminCategorias() {
  ...
  return (
    <div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5 mt-6 flex gap-3">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Perfumes, Camisas, Blusas" maxLength={50} className="flex-1 px-3 py-2 rounded-lg border border-input bg-background outline-none focus:border-primary" />
        <button onClick={save} className="bg-gradient-rose text-primary-foreground px-5 py-2 rounded-lg inline-flex items-center gap-2">
          <Plus className="h-4 w-4" /> {editing ? "Atualizar" : "Adicionar"}
        </button>
        {editing && <button onClick={() => { setEditing(null); setName(""); }} className="px-4 py-2 rounded-lg border">Cancelar</button>}
      </div>
